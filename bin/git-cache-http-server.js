// Generated by Haxe 4.0.0 (git build development @ 2a16e091d)
...
Main.authenticate = function(params,infos,callback) {
	console.log("Main.hx:58:","INFO: authenticating on the upstream repo " + infos.repo + " (user " + infos.user + ")");
...
Main.update = function(remote,local,infos,callback) {
	...
	if(!(__map_reserved[local] != null ? _this.existsReserved(local) : _this.h.hasOwnProperty(local))) {
		...
		var v = new Promise(function(resolve,reject) {
			console.log("Main.hx:70:","INFO: updating: fetching from " + infos.repo + " (user " + infos.user + ")");
...
Main.handleRequest = function(req,res) {
	try {
		...
		var infos = { repo : params.repo, user : Main.safeUser(params.auth.basic)};
		...
		Main.authenticate(params,infos,function(upRes) {
			...
			if(params.isInfoRequest) {
				Main.update(remote,local,infos,function(err) {
					if(err != null) {
						console.log("Main.hx:149:","ERR: " + err);
						console.log("Main.hx:150:",haxe_CallStack.toString(haxe_CallStack.exceptionStack()));
						res.statusCode = 500;
						res.end();
						return;
					}
					...
					up.stderr.on("data",function(data) {
						console.log("Main.hx:161:","" + params.service + " stderr: " + data);
					});
					up.on("exit",function(code) {
						if(code != 0) {
							res.end();
						}
						console.log("Main.hx:165:","INFO: " + params.service + " done with exit " + code);
					});
				});
			} else {
				res.statusCode = 200;
				...
				up1.stderr.on("data",function(data1) {
					console.log("Main.hx:179:","" + params.service + " stderr: " + data1);
				});
				up1.on("exit",function(code1) {
					if(code1 != 0) {
						res.end();
					}
					console.log("Main.hx:183:","" + params.service + " done with exit " + code1);
				});
			}
		});
	} catch( err1 ) {
		...
		console.log("Main.hx:188:","ERROR: " + Std.string(err1));
		console.log("Main.hx:189:",haxe_CallStack.toString(haxe_CallStack.exceptionStack()));
		res.statusCode = 500;
		res.end();
	}
};
Main.main = function() {
	...
	console.log("Main.hx:221:","INFO: cache directory: " + Main.cacheDir);
	console.log("Main.hx:222:","INFO: listening to port: " + Main.listenPort);
